...
parser.add_argument('--dataset', type=str, required=False, default='cve_data')
...
if __name__ == '__main__':
    ...
    acc4 = [0 for i in range(len(berts) + 1)]
    ...
    for index, true_labels in enumerate(df.label.values):
        ...
        for i, logit in enumerate(logits):
            ...
            acc2[i] += len(set(logit[:2]) & true_labels)
            acc3[i] += len(set(logit[:3]) & true_labels)
            acc4[i] += len(set(logit[:4]) & true_labels)
    ...
    for i, name in enumerate(berts + ['all']):
        ...
        with open('./results/{args.dataset}', 'a') as f:
            print("Precision@1,2,3,4,5")
            print(f'{name} {p1} {p2} {p3} {p4} {p5}', file=f)
            print(f'{name} {p1} {p2} {p3} {p4} {p5}')
